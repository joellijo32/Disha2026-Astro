...
export default function EasterEggVIP({ imageUrls = [] }: EasterEggVIPProps) {
  const [phase, setPhase] = useState<'idle' | 'ledger' | 'transitioning' | 'artifact_revealed'>(
    'idle'
  );
  ...
  const freezePage = (freeze: boolean) => {
    if (freeze) {
      ...
      if (!document.getElementById('expanded-freeze-styles')) {
        ...
        style.textContent = `body[data-expanded] #page-freeze-zone * { animation-play-state: paused !important; }`;
      ...
      window.dispatchEvent(
        new CustomEvent('freeze-page-animations', { detail: { frozen: false } })
      );
    }
  };
  ...
  useEffect(() => {
    ...
    if (!el || phase !== 'artifact_revealed') return;
  ...
  return (
    <>
      ...
      {(phase === 'ledger' || phase === 'transitioning' || phase === 'artifact_revealed') && (
      ...
      {(phase === 'transitioning' || phase === 'artifact_revealed') && (
